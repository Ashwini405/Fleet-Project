...
export default function EditStationModal({ isOpen, onClose, station, onSuccess }) {
  ...
  // Sync form fields whenever a different station is opened for editing
  useEffect(() => {
    if (station) {
      setFormData({
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
        ...
        >
          <div className="flex justify-between items-center p-5 border-b border-gray-100">
            <h3 className="text-lg font-bold text-gray-900">Edit Station</h3>
            <button
              onClick={onClose}
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 py-3 text-sm font-bold text-white bg-teal-600 hover:bg-teal-700 rounded-xl transition-colors shadow-sm shadow-teal-600/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
