...
export default function AddInventoryModal({
  ...
  useEffect(() => {
    if (editItem && isEditMode) {
      setForm({
        ...
        name: editItem.part_name || '',
        sku: editItem.sku || '',
        category: editItem.category || 'Spares',
        description: editItem.description || '',
        brand: editItem.brand || '',
        openingStock: editItem.opening_stock ?? 0,
        minStock: editItem.min_stock ?? 5,
        reorderLevel: editItem.reorder_level ?? 10,
        unit: editItem.unit || 'pcs',
        costPrice: editItem.cost_price ?? 0,
        sellingPrice: editItem.selling_price ?? 0,
      ...
      setSkuManual(true);
      setAutoSku(editItem.sku || '');
      setVehicleTags(editItem.compatible_vehicles ? JSON.parse(editItem.compatible_vehicles) : []);
      if (editItem.part_image) {
        setPartImage(editItem.part_image);
      } else {
        setPartImage(null);
        setImageFile(null);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('part_name', form.name);
      formData.append('sku', activeSku);
      formData.append('category', form.category);
      formData.append('description', form.description);
      formData.append('brand', form.brand);
      formData.append('opening_stock', form.openingStock);
      formData.append('current_stock', form.openingStock);
      formData.append('min_stock', form.minStock);
      formData.append('reorder_level', form.reorderLevel);
      formData.append('unit', form.unit);
      formData.append('cost_price', form.costPrice);
      formData.append('selling_price', form.sellingPrice);
      formData.append('inventory_value', Number(form.openingStock) * Number(form.costPrice));
      formData.append('vehicle_type', form.vehicleType);
      formData.append('compatible_vehicles', JSON.stringify(vehicleTags));
      formData.append('service_interval', form.serviceInterval);
      formData.append('preferred_vendor', form.preferredVendor);
      formData.append('gst_number', form.gst);
      formData.append('vendor_contact', form.vendorContact);
      formData.append('warehouse', form.warehouse);
      formData.append('rack_no', form.rack);
      formData.append('bin_no', form.bin);
      formData.append('expiry_date', form.expiryDate);
      formData.append('warranty', form.warranty);
      formData.append('notes', form.notes);
      formData.append('stock_status', getStockStatus(form.openingStock, form.minStock, form.reorderLevel).label);
        ...
        >
          ...
          <div className="flex border-b border-slate-100">
            {STEPS.map((s, i) => (
              <button
                key={s}
                onClick={() => setStep(i)}
                className={`flex-1 py-2.5 text-[10px] font-bold uppercase tracking-wider transition ${i === step
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="text-lg font-black text-slate-800">
                      {isEditMode ? 'Part Updated Successfully!' : 'Part Added Successfully!'}
                    </p>
                    <p className="text-xs text-slate-400 mt-1">
                      {form.name || 'Part'} has been {isEditMode ? 'updated' : 'added to inventory'}.
                    </p>
                  </div>
            ...
            {!saved && (
              <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                      ...
                      <AnimatePresence mode="wait">
                        {partImage ? (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => { setPartImage(null); setImageFile(null); }}
                          ...
                          <motion.div
                            ...
                            onDrop={handleDrop}
                            className={`flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed py-6 transition ${dragOver
                    ...
                    <div className="col-span-2">
                      <Field label="Part Name *" error={errors.name}>
                        <Input value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="e.g. Brake Pads" error={errors.name} />
                      </Field>
                    </div>

                    <Field label="Brand *" error={errors.brand}>
                      <Input value={form.brand} onChange={(e) => set('brand', e.target.value)} placeholder="e.g. Bosch" error={errors.brand} />
                    </Field>
                    ...
                    <Field label="SKU / Part Code">
                      <div className="relative">
                        <Input
                          ...
                          className={`w-full rounded-xl border px-3 py-2.5 text-sm outline-none transition font-mono ${!skuManual && autoSku ? 'border-violet-200 bg-violet-50 text-violet-700' : 'border-slate-200 bg-slate-50'
                    ...
                    <div className="col-span-2">
                      <Field label="Description">
                        <textarea
                          ...
                          className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 resize-none"
                        />
                      </Field>
                    </div>
                  </div>
                  ...
                  return (
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Opening Stock">
                        <Input type="number" min="0" value={form.openingStock} onChange={(e) => set('openingStock', Number(e.target.value))} />
                      </Field>
                      <Field label="Minimum Stock">
                        <Input type="number" min="0" value={form.minStock} onChange={(e) => set('minStock', Number(e.target.value))} />
                      </Field>
                      <Field label="Reorder Level">
                        <Input type="number" min="0" value={form.reorderLevel} onChange={(e) => set('reorderLevel', Number(e.target.value))} />
                      </Field>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              {/* Profit Margin */}
                              <div className={`rounded-lg bg-white border px-3 py-2.5 ${isProfit ? 'border-emerald-100' : 'border-red-100'
                                }`}>
                                ...
                                <AnimatePresence mode="wait">
                                  <motion.p
                                    ...
                                    exit={{ opacity: 0 }}
                                    transition={{ duration: 0.15 }}
                                    className={`text-base font-black ${isProfit ? 'text-emerald-600' : 'text-red-600'
                                      }`}
                                  >
                                    {isProfit ? '+' : ''}{fmt(profit)}
                                  </motion.p>
                                </AnimatePresence>
                                <p className={`text-[10px] mt-0.5 font-semibold ${isProfit ? 'text-emerald-500' : 'text-red-400'
                                  }`}>
                ...
                {step === 2 && (() => {
                  const suggestions = (vehicleList || [])
                    .map((v) => v.vehicle_no)
                    .filter(
                      (v) =>
                        v &&
                        v.toLowerCase().includes(tagInput.toLowerCase()) &&
                        !vehicleTags.includes(v)
                    );
                  const addTag = (val) => {
                  ...
                  return (
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <Field label="Service Interval">
                        <Input value={form.serviceInterval} onChange={(e) => set('serviceInterval', e.target.value)} placeholder="e.g. Every 10,000 km" />
                      </Field>
                            ...
                            {vehicleTags.map((tag) => (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  className="ml-0.5 rounded text-indigo-400 hover:text-red-500 transition"
                                >
                      ...
                      <Field label="GST Number">
                        <Input value={form.gst} onChange={(e) => set('gst', e.target.value)} placeholder="Vendor GST" />
                      </Field>
                      <div className="col-span-2">
                        <Field label="Vendor Contact">
                          <Input value={form.vendorContact} onChange={(e) => set('vendorContact', e.target.value)} placeholder="+91 XXXXX XXXXX" />
                        </Field>
                      </div>
                    </div>
                  );
                ...
                {step === 3 && (() => {
                  ...
                  const CATEGORY_COLORS = {
                    ...
                    Lubricants: 'bg-amber-100 text-amber-700',
                    Tyres: 'bg-slate-100 text-slate-700',
                  ...
                  return (
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <Field label="Storage Rack">
                        <Input value={form.rack} onChange={(e) => set('rack', e.target.value)} placeholder="e.g. R-A1" />
                      </Field>
                      <Field label="Bin Number">
                        <Input value={form.bin} onChange={(e) => set('bin', e.target.value)} placeholder="e.g. B-01" />
                      </Field>
                      <div className="col-span-2">
                        <Field label="Notes">
                          <textarea
                          ...
                          <div className="flex items-center gap-4 px-4 pt-4 pb-3 border-b border-indigo-100">
                            <div className="shrink-0">
                              {partImage ? (
                                ...
                                <div className="h-14 w-14 rounded-xl bg-indigo-100 border-2 border-indigo-200 flex items-center justify-center">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-black text-slate-800 truncate">
                                {form.name || <span className="text-slate-300">Part Name</span>}
                              </p>
          ...
          {!saved && (
            <div className="flex items-center justify-between gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4">
              <button onClick={onClose} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100 transition">
                Cancel
              </button>
              <div className="flex gap-2">
                {step > 0 && (
                  <button onClick={() => setStep((s) => s - 1)} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100 transition">
                    Back
                  </button>
                )}
                {step < STEPS.length - 1 ? (
                  <button
                    ...
                    className="flex items-center gap-1 rounded-xl bg-indigo-600 px-5 py-2 text-sm font-bold text-white hover:bg-indigo-700 transition"
                  >
                  ...
                  <button
                    ...
                    disabled={saving}
                    className="flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2 text-sm font-bold text-white hover:bg-emerald-700 transition disabled:opacity-70 disabled:cursor-not-allowed min-w-[130px] justify-center"
                  >
                    {saving ? (
