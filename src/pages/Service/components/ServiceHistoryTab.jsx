...
export default function ServiceHistoryTab() {
  ...
  useEffect(() => {
    fetch('http://localhost:5001/api/services')
      .then(res => res.json())
      .then(data => setPeriodicLogs(data.data || []))
      .catch(err => console.error('Error fetching periodic services:', err));

    fetch('http://localhost:5001/api/repair')
      .then(res => res.json())
      .then(data => setRepairLogs(data.data || []))
  ...
  const combinedHistory = [
    ...periodicLogs.map(l => ({
      ...
      type: l.service_type,
      status: l.status,
      id: l.id,                     // keep original numeric id
  ...
  // Filter based on search, truck, and type
                ...
                >
                  ...
                  <td className="py-2 px-2 md:py-4 md:px-4 hidden md:table-cell">
                    <span className="text-sm text-gray-600 font-medium">
                      {log.generalType === 'Periodic Service' 
                        ? `Interval: ${Number(log.interval || 0).toLocaleString()} KM` 
                  ...
                  <td className="py-2 px-2 md:py-4 md:px-4 text-center">
                    <button 
                      onClick={() => {
                        if (log.originalType === 'periodic') {
                          navigate(`/service/periodic/${log.id}`);
                        } else {
                          navigate(`/service/repair/${log.id}`);
                        }
                      }}
