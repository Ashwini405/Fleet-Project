      ...
      .then(data => {
        if (data.success) {
          const formattedVehicles = data.data.map(vehicle => ({
            ...
            wheelConfiguration: vehicle.wheel_configuration,
            totalTyres: vehicle.total_tyres,
            axleCount: vehicle.axle_count,
            layoutType: vehicle.layout_type,
            spareTyreCount: vehicle.spare_tyre_count,
            defaultTyreSize: vehicle.default_tyre_size,
            axlePositions: vehicle.axle_positions
              ? (typeof vehicle.axle_positions === 'string'
                  ? JSON.parse(vehicle.axle_positions)
                  : vehicle.axle_positions)
              : [],
            recentChanges: `${vehicle.axle_count || 0} Axles`
          }));
          setTrucks(formattedVehicles);
        }
        setLoading(false);
      })
      .catch(err => {
                  ...
                  <React.Fragment key={truck.id}>
                    ...
                    >
                      ...
                      <td className="py-2 px-2 md:py-4 md:px-4 text-center">
                        <span className="text-sm font-bold text-slate-700 font-mono">{truck.totalTyres || 0}</span>
                      </td>
