...
// ─────────────────────────────────────────────────────────────────────────────
// MountModal – updated to work with real API
// ─────────────────────────────────────────────────────────────────────────────
function MountModal({ tyre, onClose, onMounted }) {
  ...
  // Fetch vehicles and already mounted tyres for validation
  useEffect(() => {
    fetchVehicles();
    fetchMountedTyres();
  }, []);
  ...
  const fetchMountedTyres = async () => {
    try {
      const res = await axios.get('http://localhost:5001/api/tyres');
      const tyres = res.data.data || [];
      const mounted = tyres.filter(t => t.status === 'Mounted');
      setActiveTyres(mounted);
    } catch (error) {
      console.log('Fetch mounted tyres error:', error);
    }
  };
  ...
  const inputCls = (err) =>
    `w-full pl-3.5 pr-9 h-[44px] bg-white border rounded-xl text-sm font-medium text-slate-800 appearance-none focus:outline-none focus:ring-2 transition-all ${
      err ? 'border-red-300 focus:ring-red-100' : 'border-slate-200 focus:border-blue-500 focus:ring-blue-100'
    }`;
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-[#0f172a] to-[#1e293b]">
          <div>
            ...
            <p className="text-[11px] text-blue-400 font-semibold mt-0.5 font-mono">
              {tyre.id} · {tyre.make} {tyre.model}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          <div className="bg-slate-50 rounded-xl border border-slate-100 px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-slate-700">
                {tyre.make} {tyre.model}
              </p>
              <p className="text-[10px] text-slate-400 mt-0.5">
                {tyre.tyreSize} · {tyre.material}
              </p>
            </div>
            <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full ring-1 ring-blue-200">
              In Stock
            </span>
          </div>
          ...
          <div>
            <label className="block text-[10.5px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">
              Assign Vehicle <span className="text-rose-400">*</span>
            </label>
            <div className="relative">
              <select
                ...
                className={inputCls(errors.truckId)}
              >
                <option value="">Select Vehicle</option>
                {vehicles.map(v => {
                  const cnt = activeTyres.filter(t => t.vehicle_id === v.id).length;
                  const full = cnt >= (v.total_tyres || 0);
                  return (
                    <option key={v.id} value={v.id} disabled={full}>
                      {v.vehicle_no} — {v.make || v.model}
                      {full ? ' (Full)' : ` (${cnt}/${v.total_tyres || 0})`}
                    </option>
                  );
                })}
              </select>
          ...
          <div>
            <label className="block text-[10.5px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">
              Tyre Position <span className="text-rose-400">*</span>
            </label>
            <div className="relative">
              <select
                value={placement}
                onChange={e => {
                  ...
                  setErrors(p => ({ ...p, placement: '' }));
                }}
                disabled={!truckId}
                className={inputCls(errors.placement) + (!truckId ? ' opacity-40 cursor-not-allowed' : '')}
              >
                <option value="">{truckId ? 'Select Position' : 'Select vehicle first'}</option>
                {PLACEMENTS.map(p => (
                  <option key={p.id} value={p.id} disabled={occupiedPos.includes(p.id)}>
                    {p.label}
                    {occupiedPos.includes(p.id) ? ' — Occupied' : ''}
                  </option>
                ))}
              </select>
              <ChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
          ...
          <div>
            <label className="block text-[10.5px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">
              Fitted Date <span className="text-rose-400">*</span>
            </label>
            <input
              type="date"
              value={fittedDate}
              max={today()}
              onChange={e => {
                setFittedDate(e.target.value);
                setErrors(p => ({ ...p, fittedDate: '' }));
              }}
              className={`w-full px-3.5 h-[44px] bg-white border rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 transition-all ${
                errors.fittedDate
          ...
          <div className="flex items-center gap-2 text-[10px] font-semibold text-slate-400 pt-1">
            <span className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full ring-1 ring-blue-200">
              In Stock
            </span>
            <ArrowRight className="w-3 h-3" />
            <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full ring-1 ring-emerald-200">
              Active / Mounted
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2.5 px-6 py-4 border-t border-slate-100 bg-slate-50/60">
          <button
            onClick={onClose}
            className="h-10 px-5 text-sm font-bold text-slate-500 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all active:scale-95"
          >
            Cancel
          </button>
          <button
            onClick={handleMount}
            disabled={loading}
            className="h-10 px-6 text-sm font-extrabold text-white rounded-xl flex items-center gap-2 transition-all shadow-md hover:-translate-y-0.5 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 100%)' }}
          >
            <CheckCircle className="w-4 h-4" /> {loading ? 'Mounting...' : 'Confirm Mount'}
          </button>
        </div>
      </motion.div>
    </div>
...
export default function InStockTab() {
  ...
  const [vehicles, setVehicles] = useState([]); // kept for potential future use

  // Fetch stock tyres (status = 'In Stock')
  const fetchStockTyres = async () => {
    try {
      setLoading(true);
      const res = await axios.get('http://localhost:5001/api/tyres');
      const tyres = res.data.data || [];
      const inStockTyres = tyres.filter(
        tyre => String(tyre.status).trim().toLowerCase() === 'in stock'
      );
      const formatted = inStockTyres.map(tyre => {
        let files = [];
        if (tyre.tyre_files) {
          try {
            files = typeof tyre.tyre_files === 'string' ? JSON.parse(tyre.tyre_files) : tyre.tyre_files;
          } catch (e) {
            files = [];
          }
        }
        return {
          ...
          cost: Number(tyre.tyre_cost || 0),
          expectedLife: Number(tyre.expected_life_km || 0),
          files,
          dbId: tyre.id,
        };
      });
      setStockTyres(formatted);
    } catch (error) {
      console.log('Fetch Stock Tyres Error:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = useMemo(
    () =>
      stockTyres.filter(t => {
        ...
        return (
          (!search || [t.id, t.make, t.model, t.vendor || ''].some(v => v.toLowerCase().includes(q))) &&
    ...
    <div className="space-y-5">
        ...
        <div className="px-5 py-3.5 border-b border-gray-100 bg-gray-50/60 flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-[160px] max-w-xs">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Tyre No, Brand, Vendor..."
                value={search}
          ...
          {[
            ...
            ['Vendor', vendorFilter, setVendorFilter, uniqueVendors, 'All Vendors'],
        ...
        <StickyTable minWidth="900px">
          <StickyThead>
            <tr className="border-b border-gray-200 bg-slate-50 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
              <th className="py-3 px-4 whitespace-nowrap">Tyre No</th>
              ...
              <th className="py-3 px-4 whitespace-nowrap">Material</th>
              <th className="py-3 px-4 whitespace-nowrap">Vendor</th>
              <th className="py-3 px-4 whitespace-nowrap">Purchase Date</th>
              <th className="py-3 px-4 whitespace-nowrap">Cost</th>
              <th className="py-3 px-4 whitespace-nowrap">Stock Age</th>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((tyre, idx) => (
                  <motion.tr
                    ...
                    className={`border-b border-gray-100 hover:bg-blue-50/40 transition-colors duration-150 group ${
                      idx % 2 === 1 ? 'bg-gray-50/30' : 'bg-white'
                    }`}
                  >
                    <td className="py-3 px-4">
                      <span className="text-xs font-bold text-blue-600 font-mono whitespace-nowrap">{tyre.id}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-xs font-semibold text-slate-800 whitespace-nowrap">{tyre.make}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-xs text-gray-600 whitespace-nowrap">{tyre.model}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-xs font-mono text-gray-500 whitespace-nowrap">{tyre.tyreSize}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="inline-flex items-center text-[11px] font-semibold text-gray-600 bg-slate-100 px-2 py-0.5 rounded-full whitespace-nowrap">
                        {tyre.material}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-xs text-gray-600 font-medium whitespace-nowrap">{tyre.vendor || '—'}</span>
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-xs text-gray-500 tabular-nums whitespace-nowrap">{tyre.purchaseDate || '—'}</span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span className="text-xs font-semibold text-emerald-600 font-mono whitespace-nowrap">
                        {tyre.cost ? `₹${tyre.cost.toLocaleString('en-IN')}` : '—'}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <span className="text-[11px] font-semibold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full whitespace-nowrap">
                        {stockAge(tyre.purchaseDate)}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-center">
      ...
      <RegisterTyreModal
        ...
        onClose={() => {
          ...
          fetchStockTyres(); // refresh after registration
        }}
      />
      <TyreDatasheetModal
        isOpen={!!viewingTyre}
        onClose={() => setViewingTyre(null)}
        tyreData={viewingTyre}
      />
      <AnimatePresence>
        {mountingTyre && (
          <MountModal
