...
import { dummyTrucks, dummyActiveTyres, layoutPositions } from '../data/dummyData';
...
export default function RegisterTyreModal({ isOpen, onClose, onRegister, existingSerials = [] }) {
  ...
  const set = (field, value) => {
    setForm(prev => {
      ...
      // Auto-fill fitted ODO from truck's current odometer when truck is selected
      if (field === 'truckId' && prev.status === 'Mounted') {
        const truck = dummyTrucks.find(t => t.id === value);
        next.fittedOdo = truck ? String(truck.currentOdo) : '';
      }
      return next;
    });
  ...
  const truckMountedCount = form.truckId
    ? dummyActiveTyres.filter(t => t.truckNo === form.truckId).length
    : 0;
  ...
  const truckWarnMsg  = form.truckId && !truckIsFull && truckMountedCount > 0
    ? `${truckMountedCount} of ${truckCapacity} tyres already mounted on this truck`
    : null;

  const fittedOdoNum    = parseInt(form.fittedOdo) || 0;
  const expectedLifeNum = parseInt(form.expectedLife) || 0;
  // New tyre: running KM is always 0 at registration.
  // Actual running = currentOdo - fittedOdo, calculated post-registration during inspections.
  const runningKm     = 0;
  ...
  const validate = () => {
    ...
    else if (dummyActiveTyres.some(t => t.id.toUpperCase() === serialUpper))
                                                                    e.serialNo    = 'This tyre is already mounted on an active vehicle';
    ...
    else if (isMounted && form.truckId && truckIsFull)              e.truckId     = `Truck is full — ${truckMountedCount}/${truckCapacity} tyres already mounted`;
    ...
    if (isMounted && form.fittedOdo && fittedOdoNum > currentOdo) e.fittedOdo = `Cannot exceed truck current ODO (${currentOdo.toLocaleString()} km)`;
    if (isMounted && form.fittedOdo && fittedOdoNum < currentOdo * 0.5) e.fittedOdo = `Value seems too low — truck ODO is ${currentOdo.toLocaleString()} km`;
    // Invoice number
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <SectionCard icon={MapPin} title="Placement & Life">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {dummyTrucks.map(t => {
                        const mounted = dummyActiveTyres.filter(a => a.truckNo === t.id).length;
                        const full    = mounted >= t.totalTyres;
                        return (
                          <option key={t.id} value={t.id} disabled={full}>
                            {t.id} — {t.model}{full ? ' (Full)' : ` (${mounted}/${t.totalTyres})`}
                          </option>
                        );
                      })}
                    </Select>
                  ...
                  {/* Fitted ODO — only when Mounted, auto-filled from truck */}
                  {isMounted && (
                    <div>
                      ...
                      <Input
                        ...
                        readOnly={!!selectedTruck}
                        onChange={e => set('fittedOdo', e.target.value)}
                        className={`font-mono ${selectedTruck ? 'bg-slate-50 text-slate-500 cursor-default' : ''}`}
                      />
                      {selectedTruck ? (
                        <p className="mt-1 text-[10px] text-emerald-600 font-semibold flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
                          Auto-filled from truck current odometer ({currentOdo.toLocaleString()} km)
                        </p>
                      ) : (
                        <p className="mt-1 text-[10px] text-slate-400">Select a truck to auto-fill</p>
                      )}
                ...
                <SectionCard icon={ShoppingBag} title="Purchase Details">
                  ...
                  <div>
                    ...
                    {/* Range warnings — soft, not blocking */}
