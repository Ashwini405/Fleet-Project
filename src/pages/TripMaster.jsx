  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Trips Overview</h1>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-64 shadow-sm"
            />
          </div>
          <button 
        ...
        <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50">
          ...
          <div className="flex flex-wrap items-center gap-3">
             <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-lg">
                ...
                <input type="date" className="text-sm border-none focus:ring-0 p-0 text-slate-600 w-32 ml-2" />
             </div>
             
             <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-lg">
                <span className="text-sm font-medium text-slate-700">Filter by Truck:</span>
                <select className="text-sm border-none focus:ring-0 p-0 text-slate-600 font-semibold bg-transparent">
                  <option>All</option>
                  <option>AP-21-TA-1234</option>
                </select>
             </div>

             <button className="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-50 transition-colors flex items-center justify-center gap-2 font-medium">
               <FiDownload className="w-4 h-4" /> Export
             </button>

             <button 
                onClick={() => setIsGenerateTripModalOpen(true)}
                className="px-5 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 transition-colors flex items-center justify-center gap-1.5 shadow-sm"
             >
          ...
          <table className="w-full text-left whitespace-nowrap text-sm">
            <thead className="bg-slate-50 text-slate-500 font-semibold uppercase tracking-wider text-xs border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4">{activeTab === 'Active Trips' ? 'Start Date' : 'End Date'}</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {tripsToDisplay.map((trip) => (
                <tr key={trip.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-bold text-slate-800">{trip.truckNo}</td>
                  <td className="px-6 py-4 text-slate-600 font-medium">{trip.driver}</td>
                  <td className="px-6 py-4 text-slate-600">{trip.source}</td>
                  <td className="px-6 py-4 text-slate-600">{trip.destination}</td>
                  <td className="px-6 py-4 text-slate-600">{trip.distance}</td>
                  <td className="px-6 py-4 text-slate-600">{activeTab === 'Active Trips' ? trip.startDate : trip.endDate}</td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <button 
                      ...
                      className="px-4 py-1.5 text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-md hover:bg-indigo-100 transition-colors"
                    >
                      View
                    </button>
                  </td>
