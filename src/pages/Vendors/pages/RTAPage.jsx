...
const allVendors = dummyVendors.filter(v => v.category === 'rta');
...
export default function RTAPage() {
  ...
  if (selectedVendor) {
    return <RTALedger vendor={selectedVendor} onBack={() => setSelectedVendor(null)} />;
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage your RTA agents and offices</p>
        </div>
        <div className="flex items-center gap-4 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-colors"
              value={search}
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg font-bold text-sm shadow-sm transition-colors whitespace-nowrap"
          >
            <FiPlus /> Add Account
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {vendors.map(vendor => (
          <div
            ...
            className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 cursor-pointer hover:shadow-md hover:border-rose-200 transition-all group flex flex-col justify-between"
          >
            <div>
              <div className="flex justify-between items-start mb-4">
                ...
                <FiChevronRight className="text-gray-300 group-hover:text-rose-500 transition-colors" size={20} />
              </div>
              ...
              <p className="text-[11px] font-semibold text-rose-500 mb-3">{vendor.vendorCategory || 'RTA Agent'}</p>
      ...
      <AddRTAVendorModal isOpen={addOpen} onClose={() => setAddOpen(false)} />
    </div>
  );
}
