...
export default function AddOilsVendorModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.post('http://localhost:5001/api/oil-vendors', {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden" style={{ animation: 'modalSlideIn 0.3s ease-out' }}>
        ...
        <div className="flex justify-between items-center p-5 bg-gray-900">
          <div>
            <h3 className="text-sm font-bold text-white tracking-wide">Add Oils & Lubes Vendor</h3>
            <p className="text-[11px] text-amber-400 mt-0.5">Oils & Lubes Accounts · New Vendor</p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit} noValidate>
            ...
            <div className="pt-2">
              <button type="submit" disabled={loading}
                ...
                {loading ? 'Creating...' : 'Add Oils & Lubes Vendor'}
              </button>
            </div>
