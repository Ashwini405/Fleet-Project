...
const inputCls    = "w-full p-3 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 text-sm";
...
export default function AddOilsVendorModal({ isOpen, onClose, vendor = null }) {
  ...
  React.useEffect(() => {
    if (isOpen && vendor) setForm({
      name: vendor.vendor_name || '', mobile: vendor.mobile_number || '', email: vendor.email || '',
      address: vendor.address_location || '', gst: vendor.gst_number || '', openingBalance: vendor.opening_balance || '0',
      status: vendor.status || 'Active', paymentTerms: vendor.payment_terms || 'credit', bankName: vendor.bank_name || '',
      customBank: vendor.custom_bank_name || '', accountNo: vendor.account_number || '', ifsc: vendor.ifsc_code || '', upi: vendor.upi_id || '',
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (vendor) await axios.put(`http://localhost:5001/api/oil-vendors/${vendor.id}`, payload);
      else await axios.post('http://localhost:5001/api/oil-vendors', payload);
      setToast(true);
      setTimeout(() => { setToast(false); setForm(EMPTY); setErrors({}); onClose(); }, 1500);
    } catch (err) {
      console.error('OIL VENDOR SAVE ERROR:', err);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden" style={{ animation: 'modalSlideIn 0.3s ease-out' }}>
        ...
        <div className="flex justify-between items-center p-5 bg-gray-900">
          <div>
            <h3 className="text-sm font-bold text-white tracking-wide">{vendor ? 'Edit' : 'Add'} Oils & Lubes Vendor</h3>
            <p className="text-[11px] text-amber-400 mt-0.5">Oils & Lubes Accounts · {vendor ? 'Edit Vendor' : 'New Vendor'}</p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit} noValidate>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={form.name} onChange={e => set('name', e.target.value)}
                    placeholder="e.g. Castrol Distributor" className={errors.name ? inputErrCls : inputCls} />
                ...
                <div>
                  ...
                  <input type="text" value={form.address} onChange={e => set('address', e.target.value)}
                    placeholder="e.g. Ameerpet, Hyderabad" className={inputCls} />
                </div>
            ...
            <div className="pt-2">
              <button type="submit" disabled={loading}
                className="w-full py-3 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? 'Saving...' : vendor ? 'Save Changes' : 'Add Oils & Lubes Vendor'}
              </button>
            </div>
