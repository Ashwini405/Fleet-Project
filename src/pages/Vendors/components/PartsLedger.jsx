  ...
  const fetchLedger = async () => {
    try {
      ...
      const transactions = (data?.orders || []).map(order => ({
        id: order.id,
        date: order.expected_delivery || order.requested_date,
        truckId: '',
        type: 'Purchase',
        ref: order.po_number,
        desc: `${order.item_name} - ${order.vendor}`,
        debit: Number(order.total_amount || 0),
        credit: 0,
        poRef: order.po_number,
      ...
      // Map orders to PO list
      setPoList((data?.orders || []).map(order => ({
        poRef: order.po_number,
        desc: `${order.item_name} - ${order.vendor}`,
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col" style={{ animation: 'modalSlideIn 0.2s ease-out' }}>
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 shrink-0">
              <div>
                ...
                <p className="text-[11px] text-indigo-600 font-semibold mt-0.5">{vendor.vendor_name || vendor.name} · Parts & Spares</p>
              </div>
