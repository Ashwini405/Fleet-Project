...
export default function PartsLedger({ vendor, onBack }) {
  ...
  const [activeFilter, setActiveFilter] = useState('Parts Records');
  ...
  const fetchLedger = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5001/api/parts-vendors/${vendor.id}/ledger`);
  ...
  const {
    ...
    const returnTxns = rawReturns.map(item => {
      ...
      return {
        ...
        desc: `${item.part_name || 'Item'}${item.quantity_returned ? ` (Qty: ${item.quantity_returned})` : ''} - ${item.return_reason || item.notes || 'Vendor return'}`,
    ...
    const purchaseTxns = rawOrders.map(order => {
      ...
      return {
        ...
        type: 'Purchase',
        ref: poRef,
        desc: `${order.item_name || 'Parts Purchase'}${order.quantity ? ` (Qty: ${order.quantity})` : ''}`,
        debit: grossAmt,
    ...
    return (
      <div className="space-y-5 animate-fade-in pb-12">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
          <div className="animate-pulse text-gray-400">Loading parts vendor ledger...</div>
        </div>
      </div>
    );
  }
  ...
  const showOutstanding = !isCash && !isPaymentFilter && !isReturnFilter;
    ...
    <div className="space-y-5 animate-fade-in pb-12">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex-wrap gap-3">
        <button onClick={onBack} className="flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gray-800 transition-colors">
          <FiArrowLeft size={15} /> Parts &amp; Spares Accounts
        </button>
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() => {
              ...
              dlCSV(`parts-ledger-${vendor.vendor_name || vendor.name}.csv`, toCSV(filtered, cols));
            }}
          ...
          <button
            onClick={() => {
              ...
              printTbl(`Parts & Spares Ledger — ${vendor.vendor_name || vendor.name}`, cols, filtered);
            }}
            className="flex items-center gap-1.5 px-3 py-2 border border-gray-200 text-gray-500 rounded-lg font-bold text-xs hover:bg-gray-50 transition-colors"
          >
      ...
      {/* 3 Clean Finance Summary Cards (Exactly like Fuel Station) */}
        ...
        <div className="p-4 border-b border-gray-100 space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search PO reference, part item, reason..."
          ...
          <div className="flex flex-wrap gap-2 items-center justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              {(search || dateFrom || dateTo || activeFilter !== 'Parts Records') && (
                <button
                  onClick={() => { setSearch(''); setDateFrom(''); setDateTo(''); setActiveFilter('Parts Records'); setPage(1); }}
        ...
        <div className="overflow-x-auto">
          {paginated.length === 0 ? (
            ...
            <table className="w-full text-left border-collapse min-w-[820px]">
                      ...
                      <tr key={txn.id} className="hover:bg-amber-50/30 transition-colors">
                        ...
                        <td className="py-3 px-4 whitespace-nowrap">
                          <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                            <FiShoppingCart size={10} /> Purchase
                          </span>
                        </td>
              ...
              {/* Table Totals Footer (Matching 2nd Image) */}
              <tfoot>
                <tr className="border-t-2 border-gray-200 bg-gray-50/80">
                  <td colSpan={activeFilter === 'Parts Records' ? 5 : activeFilter === 'Returns' ? 5 : 5} className="py-3 px-4 text-xs font-bold text-gray-500 uppercase tracking-wide">
                    Page Totals
                  </td>
                  {activeFilter === 'Parts Records' && (
                    <>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col" style={{ animation: 'modalSlideIn 0.2s ease-out' }}>
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 shrink-0">
              <div>
                ...
                <p className="text-[11px] text-blue-600 font-semibold mt-0.5">
                  {vendor.vendor_name || vendor.name} · Parts &amp; Spares
                </p>
              </div>
            ...
            <div className="p-5 overflow-y-auto space-y-3 text-xs">
              ...
              {selectedTxn.itemName && (
                <div className="flex justify-between py-2 border-b border-gray-50">
                  <span className="font-semibold text-gray-400">Part Item</span>
              ...
              {selectedTxn.quantity && (
                <div className="flex justify-between py-2 border-b border-gray-50">
                  ...
                  <span className="font-bold text-gray-800">{selectedTxn.quantity}</span>
                </div>
              )}
              {selectedTxn.reason && (
                <div className="flex justify-between py-2 border-b border-gray-50">
