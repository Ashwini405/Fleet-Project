...
export default function OilsLedger({ vendor, onBack }) {
  ...
  const fetchLedger = async () => {
    try {
      const res = await axios.get(`http://localhost:5001/api/oil-ledger/${vendor.id}`);
      if (res.data.success) {
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => {
            ...
            dlCSV(`oils-ledger-${vendor.vendor_name}.csv`, toCSV(filtered, cols));
          ...
          <button onClick={() => {
            const cols = [{k:'date',l:'Date'},{k:'type',l:'Type'},{k:'ref',l:'Reference'},{k:'desc',l:'Description'},{k:'debit',l:'Debit'},{k:'credit',l:'Credit'},{k:'runningBalance',l:'Balance'}];
            printTbl(`Oils & Lubes Ledger — ${vendor.vendor_name}`, cols, filtered);
          }} className="flex items-center gap-1.5 px-3 py-2 border border-gray-200 text-gray-500 rounded-lg font-bold text-xs hover:bg-gray-50 transition-colors">
        ...
        <div className="overflow-x-auto">
          {paginated.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-gray-400">
              ...
              <p className="text-xs text-gray-400">{rawTxns.length === 0 ? 'Transactions will appear here once purchases are recorded.' : 'Try clearing the filters to see all transactions.'}</p>
            </div>
          ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col" style={{ animation: 'modalSlideIn 0.2s ease-out' }}>
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 shrink-0">
              <div>
                ...
                <p className="text-[11px] text-amber-600 font-semibold mt-0.5">{vendor.vendor_name} · Oils & Lubes</p>
              </div>
            ...
            <div className="p-5 overflow-y-auto space-y-0">
              {[
                ...
                ['Vendor', null, <span className="text-sm font-bold text-gray-800">{vendor.vendor_name}</span>],
