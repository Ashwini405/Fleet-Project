...
const inputCls    = "w-full p-3 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 text-sm";
...
export default function AddRTAVendorModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(err?.response?.data?.message || 'Failed to create RTA agent');
    } finally {
      setLoading(false);
    }
  };
          ...
          <form className="space-y-6" onSubmit={handleSubmit} noValidate>
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  <label className={labelCls}>Agent / Office Name <span className="text-red-400">*</span></label>
                  <input type="text" value={form.name} onChange={e => set('name', e.target.value)}
                    placeholder="e.g. RTA Agent Suresh" className={errors.name ? inputErrCls : inputCls} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="email" value={form.email} onChange={e => set('email', e.target.value)}
                      placeholder="e.g. agent@example.com" className={errors.email ? inputErrCls : inputCls} />
                    {errors.email && <p className="text-xs text-red-500 mt-1">{errors.email}</p>}
                  </div>
                </div>
                <div>
                  <label className={labelOptCls}>Office Address</label>
                  <input type="text" value={form.address} onChange={e => set('address', e.target.value)}
                    placeholder="e.g. RTO Office, Hyderabad" className={inputCls} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
            ...
            <div className="pt-2">
              <button type="submit" disabled={loading}
                className="w-full py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? 'Adding...' : 'Add RTA Agent'}
              </button>
            </div>
