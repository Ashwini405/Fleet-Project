...
const EMPTY = {
  ...
  services: [], paymentTerms: 'credit', status: 'Active',
  bankName: '', customBank: '', accountNo: '', ifsc: '', upi: '',
};

export default function EditTyreVendorModal({ isOpen, onClose, vendor }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5001/api/tyre-vendors/${vendor.id}`, {
      ...
      console.error('TYRE VENDOR UPDATE ERROR', error);
      alert(error?.response?.data?.message || 'Failed to update tyre vendor');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden" style={{ animation: 'modalSlideIn 0.3s ease-out' }}>
        ...
        <div className="flex justify-between items-center p-5 bg-gray-900">
          <div>
            <h3 className="text-sm font-bold text-white tracking-wide">Edit Tyre Vendor</h3>
            <p className="text-[11px] text-gray-400 mt-0.5">Tyres Accounts · {vendor.vendor_name}</p>
          </div>
        ...
        {toast && (
          <div className="flex items-center gap-2 px-5 py-3 bg-green-50 border-b border-green-100 text-green-700 text-sm font-semibold">
            <FiCheckCircle size={16} /> Tyre vendor updated successfully
          </div>
        )}
