          ...
          ${active ? 'bg-indigo-50' : ''}`}>
            ...
            ${done ? 'bg-green-500 text-white' : active ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
               ...
               <div>
                  ...
                  {done && s.num === 2 && formData.vehicle_no && (
                     <p className="text-[10px] text-slate-400 mt-0.5">{formData.vehicle_no}</p>
                  )}
               </div>
            </div>
         );
...
export default function AddIncidentModal({ isOpen, onClose, onSubmit }) {
   ...
   const [fd, setFd] = useState({
      ...
      // BREAKDOWN
      breakdown_category: 'Engine Failure',
      vehicle_movable: 'No',
      emergency_required: 'Yes',
      ...
      try {
         ...
         for (const file of files) {
            ...
            const response = await fetch(
               'http://localhost:5001/api/incidents/upload',
               {
                  method: 'POST',
                  body: formData
               }
   ...
   const handleSubmit = async () => {
      ...
      try {
         ...
         if (!fd.incident_location) {

            alert('Please enter incident location');
            return;
         }
                     ...
                     <div className="flex-1 overflow-y-auto px-8 py-6">
                        ...
                        {step === 2 && (
                           <div className="max-w-xl space-y-5">
                              <div className="grid grid-cols-2 gap-4">
                                 <Sel
                                    ...
                                    onChange={(e) => {
                                       ...
                                       setFd(prev => ({
                                          ...
                                          supervisor_id: selectedVehicle.supervisor_id,
                                          supervisor_name: selectedVehicle.supervisor_name || '',
                                          station_id: selectedVehicle.station_id,
                                          station_name: selectedVehicle.station_name || '',
                                          current_route: selectedVehicle.default_route || ''
                                       }));
                                    }}
                                 >
                                 ...
                                 <Inp label="Driver" required icon={User} value={fd.driver_name} readOnly />

                                 <div className="col-span-2">
                                    <Inp label="Phone" icon={Phone} value={fd.driver_phone} readOnly />
                                 </div>

                                 <Inp label="Fleet / Station" value={fd.station_name} readOnly />
                                 <Inp label="Current Route" value={fd.current_route} readOnly />
                              </div>

                              <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                                 <Info className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" />
                                 <p className="text-xs text-slate-500 font-medium">
                                    Driver and station details are fetched automatically from vehicle master.
                                 </p>
                              </div>
                           </div>
                        )}
                        ...
                        {step === 3 && (
                           <div className="max-w-xl space-y-5">
                              ...
                              <Field label="Status">
                                 <input readOnly value={fd.incident_status}
                        ...
                        {step === 4 && (
                           <div className="max-w-lg space-y-5">
                              ...
                              <Inp label="GPS Coordinates (Optional)" icon={Navigation}
                                 value={fd.gps_coordinates} onChange={e => set('gps_coordinates', e.target.value)} placeholder="13.0827, 77.5946" />
                           ...
                           <div className="max-w-xl space-y-6">
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                       ...
                                       <div>
                                          <Field label="Vehicle Movable?" required>
                                             {radioGroup('vm', 'vehicle_movable')}
                                          </Field>
                                       </div>
                                       <div>
                                          <Field label="Emergency Assistance Required?" required>
                                             {radioGroup('er', 'emergency_required')}
                                          </Field>
                                       </div>
                                    </div>
                                 </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                       ...
                                       <div>
                                          <Field label="Injury Reported?" required>
                                             {radioGroup('ir', 'injury_reported')}
                                          </Field>
                                       </div>
                                       <div>
                                          <Field label="Police Complaint?" required>
                                             {radioGroup('pc', 'police_complaint')}
                                          </Field>
                                       </div>
                                    </div>
                                 </div>
                              ...
                              {fd.type === 'Fuel Theft' && (
                                 <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                       <Inp label="Fuel Lost (Liters)" required type="number" value={fd.fuel_lost} onChange={e => set('fuel_lost', e.target.value)} placeholder="e.g. 15" />
                                       <div>
                                          <Field label="Tank Seal Broken?" required>
                                             {radioGroup('ts', 'tank_seal_broken')}
                                          </Field>
                                       </div>
                                    </div>
                                 </div>
                              ...
                              {fd.type === 'Tyre Issue' && (
                                 <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                       <Sel label="Tyre Position" required value={fd.tyre_position} onChange={e => set('tyre_position', e.target.value)}>
                                          <option value="">Select</option>
                              ...
                              {fd.type === 'Engine Failure' && (
                                 <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                       <Sel label="Failure Type" required value={fd.engine_failure_type} onChange={e => set('engine_failure_type', e.target.value)}>
                                          <option value="">Select</option>
                                       ...
                                       <div>
                                          <Field label="Emergency Assistance?" required>
                                             {radioGroup('ea2', 'emergency_required')}
                                          </Field>
                                       </div>
                                    </div>
                                 </div>
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                 ...
                                 <div className="space-y-3">
                                    {[
                                       ...
                                       { icon: Truck, label: 'Truck ID', value: fd.vehicle_no },
                                       { icon: User, label: 'Driver', value: [fd.driver_name, fd.driver_phone].filter(Boolean).join('\n') },
                                       { icon: FileText, label: 'Fleet / Station', value: fd.station_name },
                                       { icon: Navigation, label: 'Route', value: fd.current_route },
                                    ].map(r => (
                                 ...
                                 <div className="space-y-3">
                                    {[
                                       { icon: Calendar, label: 'Date & Time', value: [fd.incident_date, fd.incident_time].filter(Boolean).join(', ') },
                                       { icon: ShieldAlert, label: 'Severity / Priority', value: `${fd.severity} / ${fd.priority}` },
                                       { icon: FileText, label: 'Status', value: fd.incident_status, highlight: true },
                                       { icon: MapPin, label: 'Location', value: fd.incident_location },
                                       { icon: Navigation, label: 'GPS Coordinates', value: fd.gps_coordinates },
                                    ].map(r => (
