...
export default function AddFuelEntry({ isOpen, onClose, onSave }) {
  ...
  // ─── When vehicle changes, fetch its details ────────────────────────────
  useEffect(() => {
    if (!addForm.vehicle) {
      setVehicleInfo(null);
      return;
    ...
    fetch(`http://localhost:5001/api/vehicles/by-number/${addForm.vehicle}`)
      .then(res => res.json())
      .then(data => {
        if (data.success && data.data) {
          ...
          // ✅ FIX 3: Set vehicleId when vehicle is fetched
          setAddForm(prev => ({ ...prev, vehicleId: v.id }));
        }
      })
...
useEffect(() => {
  ...
  fetch(`http://localhost:5001/api/trips/by-vehicle/${addForm.vehicleId}`)
    .then(res => res.json())
    .then(data => {
      console.log("Trips API:", data); // 🔥 DEBUG
  ...
  // ─── Derived values (now using vehicleInfo state) ────────────────────────
  ...
  const calculatedMileage = distance > 0 && addForm.qty
    ...
    : '0.00';
  const tripInfo = trips.find(t => t.trip_id === addForm.tripId);
  const fuelRequired = tripInfo && vehicleInfo
    ? (tripInfo.est_distance / vehicleInfo.expectedMileage).toFixed(1)
    : 0;

  const canSave = addForm.vehicle && addForm.vehicleId && addForm.currentOdo && addForm.qty && addForm.rate && addForm.vendor;

  // ─── Handlers (unchanged UI logic) ───────────────────────────────────────
  ...
  const validateForm = () => {
    ...
    if (!addForm.qty) {
      newErrors.qty = 'Fuel quantity required';
  ...
  const generateSuggestions = () => {
    ...
    if (tripInfo && vehicleInfo) {
      newSuggestions.fuelRequired = `For this trip (${tripInfo.est_distance} km), suggested fuel is ${fuelRequired}L based on expected mileage`;
    }
    if (vehicleInfo && calculatedMileage !== '0.00') {
      const mileageVar = parseFloat(calculatedMileage) - vehicleInfo.expectedMileage;
      if (mileageVar < -0.5) {
  ...
  // ─── UI (unchanged except dropdowns use real data) ────────────────────────
          ...
          <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select name="vehicle" value={addForm.vehicle} onChange={handleFormChange} className={inputClass}>
              ...
              <div>
                ...
                <select name="tripId" value={addForm.tripId} onChange={handleFormChange} className={inputClass}>
                  <option value="">Select trip...</option>
                  {trips.map(t => (
                    <option key={t.trip_id} value={t.trip_id}>{t.trip_id}</option>
                  ))}
                </select>
              </div>
            </div>
          ...
          <div className="bg-indigo-50/60 rounded-xl p-4 border border-indigo-100 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div>
                ...
                <input readOnly value={vehicleInfo?.driver || ''} placeholder="Select vehicle first" className={inputAutoClass} />
              </div>
              <div>
                <label className={labelClass + " text-indigo-600"}>Previous ODO (Auto)</label>
                <input readOnly value={vehicleInfo ? prevOdo.toLocaleString() + ' km' : ''} placeholder="—" className={inputAutoClass} />
              </div>
              <div>
                <label className={labelClass + " text-indigo-600"}>Expected Mileage (Auto)</label>
                <input readOnly value={vehicleInfo ? `${vehicleInfo.expectedMileage} KMPL` : ''} placeholder="—" className={inputAutoClass} />
              </div>
              <div>
                <label className={labelClass + " text-indigo-600"}>Tank Capacity (Auto)</label>
                <input readOnly value={vehicleInfo ? `${vehicleInfo.tankCapacity} L` : ''} placeholder="—" className={inputAutoClass} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-200 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input readOnly value={vehicleInfo ? prevOdo.toLocaleString() : ''} placeholder="—" className={inputAutoClass} />
              </div>
              <div>
            ...
            {addForm.tripId && tripInfo && (
              <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-blue-600">Trip Distance:</span> {tripInfo.est_distance} km
                  </div>
                  <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={handleFormChange} 
                  className={`${inputClass} ${errors.vendor ? 'border-red-300 focus:border-red-500' : ''}`}
                >
              ...
              <div className="space-y-2">
                {alerts.map((alert, index) => (
                  <div key={index} className={`flex items-start gap-3 p-3 rounded-lg ${
                    ...
                    <div className={`text-lg ${alert.type === 'warning' ? 'text-yellow-600' : 'text-red-600'}`}>
                      {alert.icon}
                    </div>
